<x-admin-layout>
    <x-slot name="title">Edit Job Opening</x-slot>
    ...
    <div class="max-w-xl mx-auto space-y-6">
        ...
        <div class="flex items-center space-x-2 text-sm text-gray-500 dark:text-zinc-400">
            ...
            <span>/</span>
            <span class="text-gray-900 dark:text-white font-semibold">Edit Job Opening</span>
        </div>

        <div>
            <h1 class="text-3xl font-bold tracking-tight text-gray-900 dark:text-white">Edit Job: {{ $job->title }}</h1>
            <p class="text-sm text-gray-500 dark:text-zinc-400">Modify the description, requirements and closing date of this job post.</p>
        </div>

        <!-- Form Card -->
        <div class="bg-white dark:bg-zinc-900 rounded-2xl border border-gray-200 dark:border-zinc-800 shadow-sm p-6 lg:p-8">
            <form method="POST" action="{{ route('admin.careers.update', $job) }}" class="space-y-6">
                @csrf
                @method('PATCH')
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', $job->title) }}" placeholder="e.g. Video Editor" required
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="location" id="location" value="{{ old('location', $job->location) }}" placeholder="e.g. Solan, Himachal Pradesh" required
                               class="w-full bg-gray-50 dark:bg-zinc-800 border border-gray-300 dark:border-zinc-700 rounded-xl px-4 py-2.5 text-sm focus:ring-emerald-500 focus:border-emerald-500">
                    </div>
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="description" id="description" rows="3" placeholder="Brief outline of the job role..." required
                              class="w-full bg-gray-50 dark:bg-zinc-800 border border-gray-300 dark:border-zinc-700 rounded-xl px-4 py-2 text-sm focus:ring-emerald-500 focus:border-emerald-500">{{ old('description', $job->description) }}</textarea>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="requirements" id="requirements" rows="4" placeholder="e.g.&#10;• 2+ Years editing Premiere Pro"
                              class="w-full bg-gray-50 dark:bg-zinc-800 border border-gray-300 dark:border-zinc-700 rounded-xl px-4 py-2 text-sm focus:ring-emerald-500 focus:border-emerald-500">{{ old('requirements', $job->requirements) }}</textarea>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="benefits" id="benefits" rows="3" placeholder="e.g.&#10;• High-end workstation provided"
                              class="w-full bg-gray-50 dark:bg-zinc-800 border border-gray-300 dark:border-zinc-700 rounded-xl px-4 py-2 text-sm focus:ring-emerald-500 focus:border-emerald-500">{{ old('benefits', $job->benefits) }}</textarea>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="date" name="closes_at" id="closes_at" value="{{ old('closes_at', $job->closes_at ? $job->closes_at->format('Y-m-d') : '') }}"
                               class="w-full bg-gray-50 dark:bg-zinc-800 border border-gray-300 dark:border-zinc-700 rounded-xl px-4 py-2.5 text-sm focus:ring-emerald-500 focus:border-emerald-500">
                    </div>
