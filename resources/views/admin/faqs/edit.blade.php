<x-admin-layout>
    <x-slot name="title">Edit FAQ Accordion</x-slot>
    ...
    <div class="max-w-xl mx-auto space-y-6">
        ...
        <div class="flex items-center space-x-2 text-sm text-gray-500 dark:text-zinc-400">
            ...
            <span>/</span>
            <span class="text-gray-900 dark:text-white font-semibold">Edit FAQ</span>
        </div>

        <div>
            <h1 class="text-3xl font-bold tracking-tight text-gray-900 dark:text-white">Edit FAQ</h1>
            <p class="text-sm text-gray-500 dark:text-zinc-400">Modify accordion question title and response details.</p>
        </div>
            ...
            <form method="POST" action="{{ route('admin.faqs.update', $faq) }}" class="space-y-6">
                ...
                <div class="space-y-2">
                    ...
                    <select name="category" id="category" required
                        ...
                        <option value="home" {{ old('category', $faq->category) === 'home' ? 'selected' : '' }}>Home Page</option>
                        <option value="services" {{ old('category', $faq->category) === 'services' ? 'selected' : '' }}>Services Page</option>
                        <option value="contact" {{ old('category', $faq->category) === 'contact' ? 'selected' : '' }}>Contact Page</option>
                        <option value="general" {{ old('category', $faq->category) === 'general' ? 'selected' : '' }}>General / Other Page</option>
                    </select>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="question" id="question" value="{{ old('question', $faq->question) }}" placeholder="e.g. Do you work outside Himachal?" required
                           class="w-full bg-gray-50 dark:bg-zinc-800 border border-gray-300 dark:border-zinc-700 rounded-xl px-4 py-2.5 text-sm focus:ring-emerald-500 focus:border-emerald-500">
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="answer" id="answer" rows="4" placeholder="Enter the detailed answer description..." required
                              class="w-full bg-gray-50 dark:bg-zinc-800 border border-gray-300 dark:border-zinc-700 rounded-xl px-4 py-2 text-sm focus:ring-emerald-500 focus:border-emerald-500">{{ old('answer', $faq->answer) }}</textarea>
                </div>
