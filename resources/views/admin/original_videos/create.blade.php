<x-admin-layout>
    <x-slot name="title">Add Original Video</x-slot>
    ...
    <div class="max-w-xl mx-auto space-y-6">
        ...
        <div class="flex items-center space-x-2 text-sm text-gray-500 dark:text-zinc-400">
            ...
            <span>/</span>
            <span class="text-gray-900 dark:text-white font-semibold">New Original Video</span>
        </div>

        <div>
            <h1 class="text-3xl font-bold tracking-tight text-gray-900 dark:text-white">Add Original Video</h1>
            <p class="text-sm text-gray-500 dark:text-zinc-400">Add a YouTube or Instagram video to show on the Original Productions showcase page.</p>
        </div>

        <!-- Form Card -->
        <div class="bg-white dark:bg-zinc-900 rounded-2xl border border-gray-200 dark:border-zinc-800 shadow-sm p-6 lg:p-8">
            <form method="POST" action="{{ route('admin.original-videos.store') }}" enctype="multipart/form-data" class="space-y-6">
                @csrf
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title') }}" placeholder="e.g. Villages of Himachal" required
                           class="w-full bg-gray-50 dark:bg-zinc-800 border border-gray-300 dark:border-zinc-700 rounded-xl px-4 py-2.5 text-sm focus:ring-emerald-500 focus:border-emerald-500">
                </div>
                ...
                <!-- Custom Thumbnail -->
                <div class="space-y-2">
                    <label for="thumbnail" class="text-sm font-semibold text-gray-700 dark:text-zinc-300">Card Cover Thumbnail (Optional)</label>
                    <input type="file" name="thumbnail" id="thumbnail" accept="image/*"
                           class="w-full bg-gray-50 dark:bg-zinc-800 border border-gray-300 dark:border-zinc-700 rounded-xl px-4 py-2.5 text-sm focus:ring-emerald-500 focus:border-emerald-500">
                    <p class="text-[11px] text-gray-400 dark:text-zinc-550">Upload a custom banner image for this project card. If left blank, YouTube cards will auto-extract covers from the video URL.</p>
                </div>
