<x-admin-layout>
    <x-slot name="title">Edit Service: {{ $service->title }}</x-slot>
    ...
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="flex items-center space-x-2 text-sm text-gray-500 dark:text-zinc-400">
            ...
            <span>/</span>
            <span class="text-gray-900 dark:text-white font-semibold">Edit Service</span>
        </div>

        <div>
            <h1 class="text-3xl font-bold tracking-tight text-gray-900 dark:text-white">Edit Service: {{ $service->title }}</h1>
            <p class="text-sm text-gray-500 dark:text-zinc-400">Modify dynamic service features and content blocks.</p>
        </div>
            ...
            <form method="POST" action="{{ route('admin.services.update', $service) }}" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title', $service->title) }}" placeholder="e.g., Video Production" required
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="tagline" id="tagline" value="{{ old('tagline', $service->tagline) }}" placeholder="e.g., Build a Brand That People Remember" required
                               class="w-full bg-gray-50 dark:bg-zinc-800 border border-gray-300 dark:border-zinc-700 rounded-xl px-4 py-2.5 text-sm focus:ring-emerald-500 focus:border-emerald-500">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="icon" id="icon" value="{{ old('icon', $service->icon) }}" placeholder="e.g., Video, Instagram, Zap, Globe" required
                               class="w-full bg-gray-50 dark:bg-zinc-800 border border-gray-300 dark:border-zinc-700 rounded-xl px-4 py-2.5 text-sm focus:ring-emerald-500 focus:border-emerald-500">
                        <p class="text-[10px] text-gray-400">Case-sensitive Lucide icon class (e.g. <i>Instagram</i>, <i>Video</i>, <i>Zap</i>, <i>Globe</i>).</p>
                    </div>

                    <!-- Offerings Title -->
                    <div class="space-y-2">
                        <label for="offerings_title" class="text-sm font-semibold text-gray-700 dark:text-zinc-300">Offerings Checklist Title</label>
                        <input type="text" name="offerings_title" id="offerings_title" value="{{ old('offerings_title', $service->offerings_title) }}" placeholder="e.g., What We Offer" required
                               class="w-full bg-gray-50 dark:bg-zinc-800 border border-gray-300 dark:border-zinc-700 rounded-xl px-4 py-2.5 text-sm focus:ring-emerald-500 focus:border-emerald-500">
                    </div>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="short_description" id="short_description" rows="2" placeholder="Brief summary shown on homepage cards..." required
                              class="w-full bg-gray-50 dark:bg-zinc-800 border border-gray-300 dark:border-zinc-700 rounded-xl px-4 py-2.5 text-sm focus:ring-emerald-500 focus:border-emerald-500">{{ old('short_description', $service->short_description) }}</textarea>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <div class="dark:text-zinc-950">
                        <textarea name="long_description" id="long_description" class="editor">{{ old('long_description', $service->long_description) }}</textarea>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="benefits_raw" id="benefits_raw" rows="4" placeholder="e.g.&#10;Cinematic 4K Grade&#10;Viral Video Strategy"
                                  class="w-full bg-gray-50 dark:bg-zinc-800 border border-gray-300 dark:border-zinc-700 rounded-xl px-4 py-2.5 text-sm font-mono focus:ring-emerald-500 focus:border-emerald-500">{{ old('benefits_raw', $benefits_raw) }}</textarea>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="features_raw" id="features_raw" rows="4" placeholder="e.g.&#10;Script & Concept Drafting&#10;2 Shoot Days"
                                  class="w-full bg-gray-50 dark:bg-zinc-800 border border-gray-300 dark:border-zinc-700 rounded-xl px-4 py-2.5 text-sm font-mono focus:ring-emerald-500 focus:border-emerald-500">{{ old('features_raw', $features_raw) }}</textarea>
                    </div>
                </div>
                    ...
                    <div class="space-y-2">
                        ...
                        @if($service->image_path)
                            <div class="flex items-center space-x-3 mb-2">
                                <img src="{{ asset($service->image_path) }}" class="w-14 h-10 object-cover rounded border border-gray-300" alt="">
