<x-frontend-layout>
    ...
    <section class="bg-[#F8F8F8] py-20 border-b border-gray-150">
        <div class="max-w-4xl mx-auto px-6 text-center space-y-6">
            <div class="w-16 h-16 bg-white rounded-2xl flex items-center justify-center text-[#2E7D32] shadow-sm mx-auto">
        ...
        <div class="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-16">
            ...
            <div class="lg:col-span-8 space-y-12">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 border-t border-gray-100 pt-12">
                    ...
                    @if(!empty($service->benefits))
                        <div class="space-y-4">
                            <h3 class="text-lg font-bold text-gray-900 flex items-center space-x-2">
                                <span class="text-[#2E7D32]"><i data-lucide="check-circle" class="w-5 h-5"></i></span>
                                <span>Key Benefits</span>
                            </h3>
                            <ul class="space-y-2 text-xs text-gray-550 font-medium">
                                @foreach($service->benefits as $benefit)
                                    <li class="flex items-start space-x-2">
                                        <span class="text-[#2E7D32] mt-0.5">•</span>
                                        <span>{{ $benefit }}</span>
                                    </li>
                                @endforeach
                            </ul>
                    ...
                    @if(!empty($service->features))
                        <div class="space-y-4">
                            <h3 class="text-lg font-bold text-gray-900 flex items-center space-x-2">
                                <span class="text-[#2E7D32]"><i data-lucide="sliders" class="w-5 h-5"></i></span>
            ...
            <div class="lg:col-span-4 space-y-8">
                ...
                <div class="bg-[#F8F8F8] p-8 rounded-3xl border border-gray-150 space-y-6">
                    ...
                    <a href="/contact?service={{ urlencode($service->title) }}" class="inline-flex items-center justify-center space-x-2 bg-[#111111] hover:bg-[#2E7D32] text-white text-xs font-bold px-6 py-3.5 rounded-full transition w-full shadow-md">
                        ...
                        <div class="divide-y divide-gray-100 space-y-4">
                            @foreach($otherServices as $other)
                                <a href="/services/{{ $other->slug }}" class="block pt-4 first:pt-0 group">
                                    <h5 class="font-bold text-sm text-[#111111] group-hover:text-[#2E7D32] transition">{{ $other->title }}</h5>
